...
export const greetingsMail = async (email:any,) => {
  const transporter = createTrans();
...
export const purchaseMail = async (email:any,) => {
    const transporter = createTrans();
    ...
    await transporter.sendMail({
      ...
      <h1>Por eso ahora tu publicación va a permanecer destacada indefinidamente.</h1>
      <h1>¡Sentite como en Kasa!</h1>
      `
    })
  }